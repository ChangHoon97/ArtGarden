...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query("SELECT new artgarden.server.review.entity.dto.ReviewListDTO(r, COALESCE(p.posterurl,e.posterurl), COALESCE(p.name, e.name), COALESCE(p.genre, e.genre), m.nickname) " +
            "FROM Review r " +
            "LEFT OUTER JOIN Member m on r.memberid = m.loginid AND m.delyn = false " +
